...
public class Neo4jDao {
	...
	public void addNodeRelationship(Business from ,List<Business> to,BusinessRelationship raltionship){
		...
		Traverser traverser=db.traversalDescription().relationships(raltionship, Direction.OUTGOING).evaluator(Evaluators.toDepth(1)).traverse(node);
	...
	public NetGraph getNodeRelationNetGraph(long nodeId,BusinessRelationship raltionship,int outComedepth){
		Transaction tras=db.beginTx();
	...
	public NetGraph getNodeNetGraph(long nodeId){
		...
		NetGraph graph=new NetGraph();
		Node node=db.getNodeById(nodeId);
		Traverser traverser=db.traversalDescription().traverse(node);
		JSONArray nodes=new JSONArray();
		for(Node n:traverser.nodes()){
			JSONObject  nodeObj=new JSONObject();
			nodeObj.put("id", n.getId());
